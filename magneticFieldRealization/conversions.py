...
def BF2IF(vecBF: np.ndarray[float], quatBF: np.ndarray[float]):
    '''
    :param vecIF: вектор в ССК size = 3x1
    :param quatBF: кватернион ориентации КА в ССК size = 4x1
    :return: вектор в ИСК size = 3x1
    '''
    q0, q1, q2, q3 = quatBF
    # преобразуем кватернион поворота в матрицу поворота
    RotMatr = np.array([
    ...
    return np.transpose(RotMatr).dot(vecBF)


def IF2SF(vecIF: np.ndarray[float], quatSF: np.ndarray[float]):
...
def SF2IF(vecSF: np.ndarray[float], quatSF: np.ndarray[float]):
    ...
    return np.transpose(RotMatr).dot(vecSF)